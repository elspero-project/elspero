...
%{_sbindir}/semanage port -a -t bitcoin_port_t -p tcp 31756
%{_sbindir}/semanage port -a -t bitcoin_port_t -p tcp 9333
%{_sbindir}/semanage port -a -t bitcoin_port_t -p tcp 131756
%{_sbindir}/semanage port -a -t bitcoin_port_t -p tcp 31758
...
if [ $1 -eq 0 ]; then
	if [ `%{_sbindir}/sestatus |grep -c "disabled"` -eq 0 ]; then
	%{_sbindir}/semanage port -d -p tcp 31756
	%{_sbindir}/semanage port -d -p tcp 9333
	%{_sbindir}/semanage port -d -p tcp 131756
	%{_sbindir}/semanage port -d -p tcp 31758
	for selinuxvariant in %{selinux_variants}; do
