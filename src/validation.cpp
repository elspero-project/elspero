...
#if defined(NDEBUG)
# error "elspero cannot be compiled without assertions."
#endif
...
const std::string strMessageMagic = "elspero Signed Message:\n";
                              ...
                              bool fOverrideMempoolLimit, const CAmount& nAbsurdFee, std::vector<COutPoint>& coins_to_uncache)
{
...

    {
        ...
        {
            LogPrint(BCLog::MEMPOOL, "replacing tx %s with %s for %s ESC additional fees, %d delta bytes\n",
...
bool ActivateBestChain(CValidationState &state, const CChainParams& chainparams, std::shared_ptr<const CBlock> pblock) {
    ...
    do {
        ...
        bool fInitialDownload;
        {
            ...
            // Add this condition to prevent 51% attack!
...
static bool ContextualCheckBlockHeader(const CBlockHeader& block, CValidationState& state, const CChainParams& params, const CBlockIndex* pindexPrev, int64_t nAdjustedTime)
{
    ...
    if (block.GetBlockTime() <= pindexPrev->GetBlockTime() - MAX_FUTURE_BLOCK_TIME && pindexPrev->nHeight > 3500) // add this to prevent timestamp manipulation
        return state.Invalid(false, REJECT_INVALID, "time-too-old", "block's timestamp is too early");
