...
*Setup instructions for a Gitian build of Elspero Core using a Debian VM or physical system.*

Gitian is the deterministic build process that is used to build the elspero
...
- [Building Elspero Core](#building-elspero-core)
...
Clone the git repositories for elspero and Gitian.
...
in the elspero repository under 'Fetch and create inputs' to install sources which require
manual intervention. Also optionally follow the next step: 'Seed the Gitian sources cache
and offline git repositories' which will fetch the remaining files required for building
offline.

Building Elspero Core
----------------

To build Elspero Core (for Linux, OS X and Windows) just follow the steps under 'perform
Gitian builds' in [doc/release-process.md](release-process.md#perform-gitian-builds) in the elspero repository.
...
Output from `gbuild` will look something like
    ...
    Initialized empty Git repository in /home/debian/gitian-builder/inputs/elspero/.git/
...
For example:
```bash
URL=https://github.com/thrasher-/elspero.git
COMMIT=2014_03_windows_unicode_path
./bin/gbuild --commit elspero=${COMMIT} --url elspero=${URL} ../elspero/contrib/gitian-descriptors/gitian-linux.yml
./bin/gbuild --commit elspero=${COMMIT} --url elspero=${URL} ../elspero/contrib/gitian-descriptors/gitian-win.yml
./bin/gbuild --commit elspero=${COMMIT} --url elspero=${URL} ../elspero/contrib/gitian-descriptors/gitian-osx.yml
```
...
and the elspero git repository with the desired tag must both be available locally, and then gbuild must be
...
LXC_ARCH=amd64 LXC_SUITE=trusty on-target -u root \
  ...
  $( sed -ne '/^packages:/,/[^-] .*/ {/^- .*/{s/"//g;s/- //;p}}' ../elspero/contrib/gitian-descriptors/*|sort|uniq )
...
BTCPATH=/some/root/path/elspero
SIGPATH=/some/root/path/elspero-detached-sigs

./bin/gbuild --url elspero=${BTCPATH},signature=${SIGPATH} ../elspero/contrib/gitian-descriptors/gitian-win-signer.yml
```
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/elspero-linux-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/elspero-win-build.assert
    gpg --detach-sign ${VERSION}-osx-unsigned/${SIGNER}/elspero-osx-build.assert
```
...
[elspero-project/gitian.sigs.ltc](https://github.com/elspero-project/gitian.sigs.ltc/) repository, or if that's not possible create a pull
